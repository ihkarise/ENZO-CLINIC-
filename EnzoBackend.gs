 ...
 *                  searched everywhere — it is issued by the clinic's
 *                  external OPD numbering system, never generated by this
 *                  project (see requestOpdNumberFromProvider() below).
 ...
 * script-wide lock (LockService) around their availability check (or new
 * patient creation, which calls out to the external OPD provider) and the
 * write that follows it, so two requests arriving at the same instant
 * can't both pass the check and double-book the same date+slot, or both
 * create a patient row for the same new phone number.
 */
...
/* ===== External OPD Number Provider =====
 * The clinic already runs its own OPD numbering system elsewhere. This
 * project must never invent OPD numbers itself — every OPD Number stored
 * in the Patients sheet has to come from that external generator.
 *
 * This function is the ONLY place that talks to it. Swapping providers
 * (a different endpoint, auth scheme, request/response shape) means
 * editing this one function and its two config properties — nothing in
 * findOrCreatePatient(), doPost(), or ensurePatientLinks() needs to
 * change, since they only ever call requestOpdNumberFromProvider() and
 * treat its return value as an opaque string.
 *
 * Configure via Script Properties (Project Settings > Script Properties):
 *   OPD_PROVIDER_URL     required. The external generator's endpoint.
 *   OPD_PROVIDER_METHOD  optional. 'get' (default) or 'post'.
 *   OPD_PROVIDER_API_KEY optional. Sent as "Authorization: Bearer <key>".
 *
 * Contract: returns a non-empty OPD Number string on success, or throws.
 * Callers MUST NOT create a Patients row unless this call succeeds — a
 * patient is never written with a blank/placeholder OPD Number. */
function requestOpdNumberFromProvider(patient){
  const props = PropertiesService.getScriptProperties();
  const url = props.getProperty('OPD_PROVIDER_URL');
  if(!url) throw new Error('OPD provider is not configured (set the OPD_PROVIDER_URL script property)');
  const method = (props.getProperty('OPD_PROVIDER_METHOD') || 'get').toLowerCase();
  const apiKey = props.getProperty('OPD_PROVIDER_API_KEY');
  const options = {
  ...
  if(method === 'post'){
    ...
    options.payload = JSON.stringify({ name: (patient && patient.name) || '', phone: (patient && patient.phone) || '' });
  }
  let resp;
  try{
    resp = UrlFetchApp.fetch(requestUrl, options);
  }catch(err){
    throw new Error('OPD provider request failed: ' + err.message);
  }
  const code = resp.getResponseCode();
  if(code < 200 || code >= 300) throw new Error('OPD provider returned HTTP ' + code);
  return parseOpdProviderResponse(resp.getContentText());
}

/** Pulled out as its own function (no UrlFetchApp/Properties calls) purely
 *  so the response-parsing contract can be unit-tested outside Apps
 *  Script. Accepts a handful of reasonable field-name shapes since we
 *  don't control the external system's exact response format. */
function parseOpdProviderResponse(bodyText){
  let data;
  try{ data = JSON.parse(bodyText); }catch(err){ throw new Error('OPD provider returned invalid JSON'); }
  const opd = data && (data.opdNumber || data.opd_number || data.opd || data.OPDNumber
    || (data.data && (data.data.opdNumber || data.data.opd_number || data.data.opd)));
  if(!opd) throw new Error('OPD provider response is missing an OPD number');
  return String(opd);
}
 ...
 *
 *  Throws if a brand-new patient needs to be created and the external OPD
 *  provider fails — callers must let that propagate (or catch it and
 *  abort the whole action) rather than writing a Patients row with no OPD
 *  Number. Existing patients (providedId or phone match) never touch the
 *  provider at all. */
function findOrCreatePatient(name, phone, providedId){
  ...
  const opd = requestOpdNumberFromProvider({ name: name, phone: phone }); // throws — no row is written below if this fails
 ...
 *  patient (matched by phone, else created fresh via the external OPD
 *  provider) without touching any other cell, and without ever creating
 *  two patients for the same phone number. The three sheet writes stay
 *  batched (not one write per row) — but a brand-new patient still means
 *  one external OPD provider call per unique new phone/blank-phone row,
 *  since that number cannot be generated locally or guessed in advance.
 *  Caller must hold the script lock.
 *
 *  If the provider fails partway through, this throws and NONE of the
 *  three setValues() calls below have run yet (they only happen after
 *  both loops finish), so nothing is left half-migrated — the next call
...
function ensurePatientLinks(apptSheet, onlineSheet){
  ...
  function getOrCreate(name, phone){
    ...
    const opd = requestOpdNumberFromProvider({ name: name, phone: phone }); // throws — see doc comment above
    const patientId = 'pt' + Utilities.getUuid().slice(0, 10);
    const now = new Date();
 ...
 *
 *  If the external OPD provider is down/misconfigured, ensurePatientLinks()
 *  throws before writing anything (see its doc comment) — this is caught
 *  here so doGet doesn't fail outright; the migrated flag is simply left
 *  unset and the next request retries the whole migration from scratch. */
function ensureMigrated(){
  ...
  try{
    ...
    Logger.log('ensureMigrated: OPD provider failure, will retry next request — ' + err.message);
  } finally {
    lock.releaseLock();
  }
}
...
/* ===== ONE-TIME SETUP: point at the clinic's external OPD generator =====
 * Edit the URL (and method/key if the provider needs them) below, run
 * once, then delete this function. See requestOpdNumberFromProvider()
 * for the full contract this endpoint must satisfy. */
function setOpdProviderConfig(){
  ...
  // props.setProperty('OPD_PROVIDER_API_KEY', 'YOUR_API_KEY'); // only if the provider requires auth
}
 ...
 *  branches below: turns a thrown OPD-provider failure into the same
 *  { ok:false, error:'opd_provider_failed', message } shape every branch
 *  returns to the client, instead of each branch duplicating a try/catch.
 *  Returns { patientId } on success or { errorResponse } on failure —
 *  callers must check errorResponse first and `return` it as-is. */
function resolvePatient(name, phone, providedId){
  try{
    return { patientId: findOrCreatePatient(name, phone, providedId) };
  }catch(err){
    return { errorResponse: json({ ok:false, error:'opd_provider_failed', message: err.message }) };
  }
}
...
function doPost(e){
  ...
  if(p.action === 'createPatient'){
    ...
    try{
      ...
      let opd;
      try{
        opd = requestOpdNumberFromProvider({ name: p.name, phone: p.phone });
      }catch(err){
        return json({ ok:false, error:'opd_provider_failed', message: err.message });
      }
      const patientId = 'pt' + Utilities.getUuid().slice(0, 10);
      const now = new Date();
