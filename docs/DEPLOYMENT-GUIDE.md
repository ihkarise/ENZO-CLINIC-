...
| B | OPD Number | Text | issued by the clinic's external OPD numbering system (never generated by this app), never edited |
| C | Name | Text | |
| D | Phone | Text | |
...
### 6.6a Configure the external OPD number provider (required before patients can be created)

This app never generates OPD numbers itself — every OPD Number comes from
the clinic's existing external OPD numbering system. This must be
configured before anyone can book an appointment for a new patient or add
an online record for one, since patient creation calls out to it.

1. In the Apps Script editor, find `setOpdProviderConfig()` near
   `setCredentials()`:
   ```js
   function setOpdProviderConfig(){
     const props = PropertiesService.getScriptProperties();
     props.setProperty('OPD_PROVIDER_URL', 'https://YOUR-OPD-GENERATOR-ENDPOINT');
     props.setProperty('OPD_PROVIDER_METHOD', 'get'); // or 'post'
     // props.setProperty('OPD_PROVIDER_API_KEY', 'YOUR_API_KEY'); // only if required
   }
   ```
2. Replace the URL with the clinic's real OPD-generator endpoint. Set the
   method to however that endpoint expects to be called, and uncomment
   the API key line if it requires authentication (sent as
   `Authorization: Bearer <key>`).
3. Select `setOpdProviderConfig` in the function dropdown and click
   **Run** (same authorization flow as §6.5 if prompted).
4. Confirm it ran (`Execution completed`, no red error text), then delete
   or comment out `setOpdProviderConfig()` the same way you did
   `setCredentials()` in §6.6 — it stores the endpoint in Script
   Properties, it doesn't need to run again.

The provider is expected to return JSON containing the OPD number under
`opdNumber` (or `opd_number`/`opd`/`OPDNumber`, top-level or nested under
`data`). If the endpoint's shape differs, adjust
`parseOpdProviderResponse()` in `EnzoBackend.gs` — that is the only place
response parsing happens.

**If this isn't configured (or the provider is unreachable), patient
creation fails with a clear error instead of silently creating a patient
with no OPD Number** — see §7's QA checklist below to verify this.
...
- [ ] Confirm §6.6a is done (`OPD_PROVIDER_URL` configured and reachable)
      **before** running the checks below — every one of them creates at
      least one new patient, which calls out to the external OPD provider.
...
- [ ] Book a **new** patient (a phone number you haven't used before).
      Confirm a new row appeared in `Patients` with an OPD Number that
      matches what your external OPD system actually returned (not a
      number this app made up), and that the appointment's row in
      `Appointments` has a matching value in column U.
...
- [ ] Start a third booking with that same phone number, but this time
      ...
      Number (again from the external provider) is created, and the new
      appointment links to it, not the first patient.
...
- [ ] Temporarily set `OPD_PROVIDER_URL` to something unreachable (a typo)
      and try booking a brand-new patient. Confirm: no row is added to
      `Patients`, no appointment is created, and reception sees a clear
      error rather than a silently broken/blank OPD Number. Restore the
      correct URL afterwards.
- [ ] If you're upgrading a clinic that already had appointments before
      ...
      one row per distinct phone number that existed before, each with a
      real OPD Number from the external provider.
...
- [ ] `Patients` tab exists in the Sheet with a header row, `OPD_PROVIDER_URL`
      (§6.6a) is configured and reachable, and at least one real OPD Number
      has come back from the external provider via a real test booking
