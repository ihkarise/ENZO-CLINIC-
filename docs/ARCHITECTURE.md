...
patient a permanent identity (an internal Patient ID + an OPD Number issued
by the clinic's external OPD numbering system) instead of grouping
appointments by guessing name/phone, adds booking-time duplicate
...
| B | OPD Number | issued by the clinic's external OPD numbering system via `requestOpdNumberFromProvider()` (never generated by this app), never edited; what staff actually see/search |
| C | Name | |
| D | Phone | |
...
| POST | `createPatient` | token | `createPatient` | **(Phase 3)** Fetches an OPD Number from the external OPD provider, then unconditionally appends a Patients row — no dedup check, that's the caller's decision (e.g. "Create new anyway"). Fails with `opd_provider_failed` and creates nothing if the provider call fails |
...
- **OPD Number** (`ENZO-000123` or whatever format the clinic's system
  uses) — issued by the clinic's external OPD numbering system via
  `requestOpdNumberFromProvider()` in `EnzoBackend.gs`; this project never
  generates one itself. Fetched under the script lock as part of creating
  a new patient, so two simultaneous bookings for a genuinely new phone
  number can't race each other into creating two Patients rows. If the
  provider call fails, `findOrCreatePatient()`/`requestOpdNumberFromProvider()`
  throw before any row is written, and every `doPost` branch that can reach
  that path (`book`/`update`/`online`/`complete`/`createPatient`) catches it
  and returns `{ ok:false, error:'opd_provider_failed', message }` instead
  of creating a patient with no OPD Number. Never edited by the app once
  stored.
...
2. Scans `Appointments` once; any row with a blank Patient ID is matched
   against that map, or — if no match — a new patient is queued, which
   means one `requestOpdNumberFromProvider()` call per distinct new/blank
   phone found (the external OPD provider can't be batched; each new
   patient needs its own real OPD Number). Collects the *entire* column's
   new values into one in-memory array.
...
previously-blank Patient ID column. If the OPD provider fails partway
through, `ensurePatientLinks()` throws before any of the three
`setValues()` calls run — `ensureMigrated()` catches that, leaves the flag
unset, and the next request simply retries the whole migration from
scratch (safe because nothing was written yet).
